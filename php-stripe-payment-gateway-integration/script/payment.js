// You must include publishable key in your JavaScript file when using the Stripe.js library.
// It is used in the client side code. For server side we use secret key. So will add it in PHP.
Stripe.setPublishableKey('pk_test_51MU3N6H5reGvJKwGaJtqFTmW7AmQoqALvx9z8Gfy6GTwnIP2OrJoNEN56UQwQt2wk8oRaXlVUmZDiAdTtmAw2Iys00EQsclv4w');


// This function runs on pay now button on click.
function stripePay(event) {
    event.preventDefault();
    // validateform function returns true if all fields are valid. 
    if(validateForm() == true) {
     $('#payNow').attr('disabled', 'disabled');
     $('#payNow').val('Payment Processing....');
     // Stripe.createToken is function from stripe API that takes an object as an argument and a callback functions.
     Stripe.createToken({
...
function stripeResponseHandler(status, response) {
 if(response.error) {
  ...
  var stripeToken = response['id']; // Get stripe Token
  // Token added in input so we will use in PHP for server side.
  $('#paymentForm').append("<input type='hidden' name='stripeToken' value='" + stripeToken + "' />");

  // Form is being submitted and now we will move to process.php
  $('#paymentForm').submit();
 }
}
...
function validateForm() {
 ...
 // Jquery libaray being used for credit card validation.
...
// Function to validate card month, year and CVC
...
// Function to add auto spaces while user inputs in card number field.
